...
function roll(){
    ...
    $sql = "SELECT started FROM rollcall WHERE committee LIKE 'UNGA-ECOFIN'";
    $a=$link->query($sql);
    $r=$a->fetch_assoc();
...
$sql = "SELECT * FROM `ebvUNGA-ECOFIN`";
$result = $link->query($sql);
if ($result->num_rows>0){
    $e.=<<<'EOT'
...
if(isset($_POST['submit'])){
    ...
    $totalt=$_POST['total'];
    $link = new mysqli("localhost","u864523831_cvmun20","ChettinadVidyashram600028","u864523831_cvmun20");
    $link->query("DELETE FROM `ebvUNGA-ECOFIN`");
    $sql = "INSERT INTO `ebvUNGA-ECOFIN`(motion,name,type,minutes,totalt) VALUES('$motion','$name','$type','$minutes','$totalt')";
    $link->query($sql);
    $link->query("DELETE FROM `vUNGA-ECOFIN`");
    header('Location:/eb/ecofin/vote/');
}
if(isset($_POST['submit2'])){$link->query("UPDATE `ebvUNGA-ECOFIN` SET `end`='yes'");header('Location:/eb/ecofin/vote/');}
?>
<!doctype html>
